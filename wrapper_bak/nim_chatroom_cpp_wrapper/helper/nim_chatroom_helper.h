...
struct NIM_CHATROOM_SDK_CPPWRAPPER_DLL_API ChatRoomNotification {
    ...
    void ParseFromJsonValue(const nim_cpp_wrapper_util::Json::Value& values) {
        ...
        if (id_ == kNIMChatRoomNotificationIdMemberTagChanged) {
            if (values[kChatRoomNotificationKeyData].isMember(kChatRoomNotificationDataKeyTags)) {
                if (values[kChatRoomNotificationKeyData][kChatRoomNotificationDataKeyTags].isArray()) {
                    target_tag_ =
                        nim_cpp_wrapper_util::Json::FastWriter().write(values[kChatRoomNotificationKeyData][kChatRoomNotificationDataKeyTags]);
                }
            }
        }
    }
...
struct NIM_CHATROOM_SDK_CPPWRAPPER_DLL_API ChatRoomGetMsgHistoryByTagsParameters {
    ...
    std::string ToJsonString() const {
        ...
        nim_cpp_wrapper_util::Json::Value tags_value;
        for (auto& tag : tags_)
            tags_value.append(tag);
        values[kNIMChatRoomGetMsgHistoryKeyTags] = writer.write(tags_value);
        values[kNIMChatRoomGetMsgHistoryKeyStartTime] = start_timetag_;
        values[kNIMChatRoomGetMsgHistoryKeyEndTime] = end_timetag_;
        values[kNIMChatRoomGetMsgHistoryKeyLimit] = limit_;
        values[kNIMChatRoomGetMsgHistoryKeyReverse] = reverse_;
        auto it = msg_types_.begin();
        while (it != msg_types_.end()) {
