...
struct NIM_QCHAT_SDK_CPPWRAPPER_DLL_API QChatDefaultAttach : public IQChatAttachInterface {
    ...
    void SetRawData(const std::string& raw_data) override { msg_attach = raw_data; };
...
struct NIM_QCHAT_SDK_CPPWRAPPER_DLL_API QChatMessageUpdateInfo {
    ...
    uint32_t operator_client_type{0};
...
struct NIM_QCHAT_SDK_CPPWRAPPER_DLL_API QChatMessageSend {
    ...
    QChatMessageSend(const NIMQChatMessage& c_param) {
        ...
        mention_accids.clear();
        for (auto index = 0; index < c_param.mention_accids_count; ++index) {
            mention_accids.emplace_back(c_param.mention_accids[index]);
        }
        mention_role_ids.clear();
        for (auto index = 0; index < c_param.mention_role_ids_count; ++index) {
            mention_role_ids.emplace_back(c_param.mention_role_ids[index]);
        }
    ...
    NIMQChatMessage ToCParam() const {
        ...
        c_param.mention_accids_count = mention_accids.size();
        c_mention_accids.clear();
        for (auto& accid : mention_accids) {
            c_mention_accids.emplace_back(const_cast<char*>(accid.c_str()));
        }
        c_param.mention_accids_count = c_mention_accids.size();
        c_param.mention_accids = const_cast<char**>(c_mention_accids.data());
        c_param.mention_role_ids_count = mention_role_ids.size();
        c_param.mention_role_ids = const_cast<uint64_t*>(mention_role_ids.data());
...
protected:
    mutable std::vector<char*> c_mention_accids;
};
...
struct NIM_QCHAT_SDK_CPPWRAPPER_DLL_API QChatMessage : public QChatMessageSend {
    ...
    QChatMessage(const NIMQChatMessage& c_param)
        ...
        accids_of_mentioned_roles_available = c_param.accids_of_mentioned_roles_available;
        accids_of_mentioned_roles.clear();
        for (auto index = 0; index < c_param.accids_of_mentioned_roles_count; ++index) {
            accids_of_mentioned_roles.emplace_back(c_param.accids_of_mentioned_roles[index]);
        }
    }
    NIMQChatMessage ToCParam() const {
        ...
        c_param.accids_of_mentioned_roles_available = accids_of_mentioned_roles_available;
        c_accids_of_mentioned_roles.clear();
        for (auto& accid : accids_of_mentioned_roles) {
            c_accids_of_mentioned_roles.emplace_back(const_cast<char*>(accid.c_str()));
        }
        c_param.accids_of_mentioned_roles = const_cast<char**>(c_accids_of_mentioned_roles.data());
        c_param.accids_of_mentioned_roles_count = c_accids_of_mentioned_roles.size();
        return c_param;
    }
    // 内部字段
    /// @身份组所包含的accid列表是否有效
...
protected:
    mutable std::vector<char*> c_accids_of_mentioned_roles;
};
...
struct NIM_QCHAT_SDK_CPPWRAPPER_DLL_API QChatAreMentionedMeMessagesResp {
    /// 操作结果, 参考NIMResCode
    uint32_t res_code{0};
    ...
    QChatAreMentionedMeMessagesResp(const NIMQChatAreMentionedMeMessagesResp& c_resp) {
        ...
        if (c_resp.res_code != kNIMResSuccess)
            return;
        for (size_t i = 0; i < c_resp.result_count; i++) {
            result.emplace(c_resp.result[i].msg_id, c_resp.result[i].mentioned_me);
        }
    }
    NIMQChatAreMentionedMeMessagesResp ToCParam() const {
        NIMQChatAreMentionedMeMessagesResp c_response;
        c_response.res_code = res_code;
        c_result_.clear();
        for (auto&& iter : result) {
            NIMQChatMentionedResult result;
            result.msg_id = const_cast<char*>(iter.first.c_str());
            result.mentioned_me = iter.second;
            c_result_.push_back(result);
        }
        c_response.result_count = static_cast<uint32_t>(c_result_.size());
        c_response.result = const_cast<NIMQChatMentionedResult*>(c_result_.data());
        return c_response;
    }

private:
    mutable std::vector<NIMQChatMentionedResult> c_result_;
};
...
struct NIM_QCHAT_SDK_CPPWRAPPER_DLL_API QChatGetQuickCommentsResp {
    ...
    uint32_t res_code{0};
    /// 快捷评论列表
    std::vector<QChatQuickCommentInfo> comments{};
...
/// @struct QChatGetMentionedMeMessagesResp
...
struct QChatGetMentionedMeMessagesParam {
    ...
    QChatGetMentionedMeMessagesParam(const NIMQChatGetMentionedMeMessagesParam& param) {
        ...
        timestamp = param.timestamp;
        limit = param.limit;
        cb = [c_callback = param.cb, user_data = param.user_data](const QChatGetMentionedMeMessagesResp& response) {
            auto c_response = response.ToCParam();
            c_response.user_data = user_data;
            c_callback(&c_response);
        };
    ...
    NIMQChatGetMentionedMeMessagesParam ToCParam() const {
        auto cb_holder = new GetMentionedMeMessagesCallback(cb);
        NIMQChatGetMentionedMeMessagesParam param = {};
        param.server_id = server_id;
        param.channel_id = channel_id;
        param.timestamp = timestamp;
        param.limit = limit;
        param.user_data = cb_holder;
        param.cb = [](const NIMQChatGetMentionedMeMessagesResp* resp) {
            InvokeCallback<GetMentionedMeMessagesCallback, NIMQChatGetMentionedMeMessagesResp>(resp);
        };
        return param;
    }
};
...
struct NIM_QCHAT_SDK_CPPWRAPPER_DLL_API QChatAreMentionedMeMessagesParam {
    /// 查询消息是否 @ 当前用户的异步回调
    AreMentionedMeMessagesCallback cb{nullptr};
    /// 消息列表
    std::vector<QChatMessage> messages;
    QChatAreMentionedMeMessagesParam() = default;
    QChatAreMentionedMeMessagesParam(const NIMQChatAreMentionedMeMessagesParam& param) {
        for (size_t i = 0; i < param.message_count; i++) {
            messages.push_back(param.messages[i]);
        }
        cb = [c_callback = param.cb, user_data = param.user_data](const QChatAreMentionedMeMessagesResp& response) {
            auto c_response = response.ToCParam();
            c_response.user_data = user_data;
            c_callback(&c_response);
        };
    ...
    NIMQChatAreMentionedMeMessagesParam ToCParam() const {
        auto cb_holder = new AreMentionedMeMessagesCallback(cb);
        c_messages.clear();
        for (auto& message : messages) {
            c_messages.push_back(message.ToCParam());
        }
        NIMQChatAreMentionedMeMessagesParam param = {};
        param.messages = const_cast<NIMQChatMessage*>(c_messages.data());
        param.message_count = c_messages.size();
        param.user_data = cb_holder;
        param.cb = [](const NIMQChatAreMentionedMeMessagesResp* resp) {
            InvokeCallback<AreMentionedMeMessagesCallback, NIMQChatAreMentionedMeMessagesResp>(resp);
        };
        return param;
    }
...
struct NIM_QCHAT_SDK_CPPWRAPPER_DLL_API QChatMessageSearchPageParam {
    ...
    /// 排序规则
    NIMQChatMessageSearchSort sort{kNIMQChatMessageSearchSortCreateTime};
    /// 查询条数
    uint32_t limit{100};
    /// 查询游标, 查询的起始位置
    std::string cursor;
