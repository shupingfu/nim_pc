...
static inline double integerToDouble(nim_cpp_wrapper_util::Json::UInt64 value) {
...
static inline char* duplicateStringValue(const char* value, size_t length) {
    ...
    if (newString == nullptr) {
        throwRuntimeError(
            "in nim_cpp_wrapper_util::Json::Value::duplicateStringValue(): "
...
static inline char* duplicateAndPrefixStringValue(const char* value, unsigned int length) {
    ...
    JSON_ASSERT_MESSAGE(length <= static_cast<unsigned>(Value::maxInt) - sizeof(unsigned) - 1U,
        "in nim_cpp_wrapper_util::Json::Value::duplicateAndPrefixStringValue(): "
...
Value::CZString::CZString(const CZString& other) {
    cstr_ =
        (other.storage_.policy_ != noDuplication && other.cstr_ != nullptr ? duplicateStringValue(other.cstr_, other.storage_.length_) : other.cstr_);
    storage_.policy_ =
...
const char* Value::asCString() const {
    JSON_ASSERT_MESSAGE(type() == stringValue, "in nim_cpp_wrapper_util::Json::Value::asCString(): requires stringValue");
...
void Value::clear() {
    JSON_ASSERT_MESSAGE(type() == nullValue || type() == arrayValue || type() == objectValue,
        "in nim_cpp_wrapper_util::Json::Value::clear(): requires complex value");
    start_ = 0;
    limit_ = 0;
    switch (type()) {
        case arrayValue:
...
void Value::resize(ArrayIndex newSize) {
    JSON_ASSERT_MESSAGE(type() == nullValue || type() == arrayValue, "in nim_cpp_wrapper_util::Json::Value::resize(): requires arrayValue");
    if (type() == nullValue)
...
Value& Value::operator[](ArrayIndex index) {
    JSON_ASSERT_MESSAGE(
        type() == nullValue || type() == arrayValue, "in nim_cpp_wrapper_util::Json::Value::operator[](ArrayIndex): requires arrayValue");
    if (type() == nullValue)
        *this = Value(arrayValue);
    CZString key(index);
...
Value& Value::operator[](int index) {
    JSON_ASSERT_MESSAGE(index >= 0, "in nim_cpp_wrapper_util::Json::Value::operator[](int index): index cannot be negative");
    return (*this)[ArrayIndex(index)];
}

const Value& Value::operator[](ArrayIndex index) const {
    JSON_ASSERT_MESSAGE(
        type() == nullValue || type() == arrayValue, "in nim_cpp_wrapper_util::Json::Value::operator[](ArrayIndex)const: requires arrayValue");
    if (type() == nullValue)
...
const Value& Value::operator[](int index) const {
    JSON_ASSERT_MESSAGE(index >= 0, "in nim_cpp_wrapper_util::Json::Value::operator[](int index) const: index cannot be negative");
    return (*this)[ArrayIndex(index)];
}
...
Value& Value::resolveReference(const char* key) {
    JSON_ASSERT_MESSAGE(
        type() == nullValue || type() == objectValue, "in nim_cpp_wrapper_util::Json::Value::resolveReference(): requires objectValue");
    if (type() == nullValue)
...
Value& Value::resolveReference(char const* key, char const* end) {
    JSON_ASSERT_MESSAGE(
        type() == nullValue || type() == objectValue, "in nim_cpp_wrapper_util::Json::Value::resolveReference(key, end): requires objectValue");
    if (type() == nullValue)
        *this = Value(objectValue);
...
Value const* Value::find(char const* begin, char const* end) const {
    ...
    JSON_ASSERT_MESSAGE(type() == nullValue || type() == objectValue,
        "in nim_cpp_wrapper_util::Json::Value::find(begin, end): requires "
...
Value* Value::demand(char const* begin, char const* end) {
    JSON_ASSERT_MESSAGE(type() == nullValue || type() == objectValue,
        "in nim_cpp_wrapper_util::Json::Value::demand(begin, end): requires "
        "objectValue or nullValue");
...
Value& Value::append(Value&& value) {
    JSON_ASSERT_MESSAGE(type() == nullValue || type() == arrayValue, "in nim_cpp_wrapper_util::Json::Value::append: requires arrayValue");
    if (type() == nullValue) {
        *this = Value(arrayValue);
    }
...
bool Value::insert(ArrayIndex index, Value&& newValue) {
    JSON_ASSERT_MESSAGE(type() == nullValue || type() == arrayValue, "in nim_cpp_wrapper_util::Json::Value::insert: requires arrayValue");
...
void Value::removeMember(const char* key) {
    JSON_ASSERT_MESSAGE(type() == nullValue || type() == objectValue, "in nim_cpp_wrapper_util::Json::Value::removeMember(): requires objectValue");
    if (type() == nullValue)
        return;
...
Value::Members Value::getMemberNames() const {
    JSON_ASSERT_MESSAGE(
        type() == nullValue || type() == objectValue, "in nim_cpp_wrapper_util::Json::Value::getMemberNames(), value must be objectValue");
    if (type() == nullValue)
...
void Value::setComment(String comment, CommentPlacement placement) {
    ...
    JSON_ASSERT_MESSAGE(comment[0] == '\0' || comment[0] == '/', "in nim_cpp_wrapper_util::Json::Value::setComment(): Comments must start with /");
...
String Value::toStyledString() const {
    ...
    out += nim_cpp_wrapper_util::Json::writeString(builder, *this);
    out += '\n';
