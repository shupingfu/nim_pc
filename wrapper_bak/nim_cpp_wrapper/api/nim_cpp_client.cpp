...
typedef void (*nim_client_login)(const char* app_token,
    const char* account,
    const char* password,
...
typedef void (
    *nim_client_reg_sync_multiport_push_config_cb)(const char* json_extension, nim_client_multiport_push_config_cb_func cb, const void* user_data);
...
typedef void (*nim_client_get_server_current_time)(bool calc_local,
...
static void CallbackLogin(const char* json_res, const void* callback) {
    CallbackProxy::DoSafeCallback<Client::LoginCallback>(callback, [=](const Client::LoginCallback& cb) {
        ...
        if (nim::ParseJsonValue(PCharToString(json_res), values) && values.isObject()) {
...
static void CallbackLogout(const char* json_res, const void* callback) {
    CallbackProxy::DoSafeCallback<Client::LogoutCallback>(
        callback,
        [=](const Client::LogoutCallback& cb) {
...
void CallbackGetServerCurrentTime(int rescode, bool calc_local, uint64_t time, const void* callback) {
    CallbackProxy::DoSafeCallback<Client::GetCurrentServerTimeCallback>(
        callback,
        [=](const Client::GetCurrentServerTimeCallback& cb) {
...
bool Client::Init(const std::string& app_key, const std::string& app_data_dir, const std::string& app_install_dir, const SDKConfig& config) {
#ifdef NIM_SDK_DLL_IMPORT
...
#include "TargetConditionals.h"
    ...
    static const char* kSdkNimDll = "libnim.dylib";
#else
...
#error "Unknown compiler"
    ...
    config_values[nim::kNIMCustomClientType] = config.custom_client_type_;
    ...
    config_values[nim::kNIMCacheSessionDataWhenDelete] = config.cache_session_data_when_delete_;
    config_values[nim::kNIMSDKType] = config.sdk_type;
    config_values[nim::kNIMSDKHumanVersion] = config.sdk_human_version;
    config_values[nim::kNIMMockRefer] = config.mock_refer_;
    config_values[nim::kNIMMockUA] = config.mock_ua_;
    config_values[nim::kNIMFcsAuthType] = config.fcs_auth_type_;
    config_values[nim::kNIMFcsCustomEnable] = config.custom_enable_fcs_;
    config_values[nim::kNIMSessionIgnoreMsgTypes] = nim_cpp_wrapper_util::Json::Value(nim_cpp_wrapper_util::Json::arrayValue);
    for (auto it : config.session_ignore_msg_types_) {
        config_values[nim::kNIMSessionIgnoreMsgTypes].append((int)it);
    }
    ...
    auto config_root_str = GetJsonStringWithNoStyled(config_root);
    auto ret = NIM_SDK_GET_FUNC(nim_client_init)(app_data_dir.c_str(), app_install_dir.c_str(), config_root_str.c_str());
...
void Client::Cleanup2(const std::string& json_extension /*= ""*/) {
    ...
    NIM_SDK_GET_FUNC(nim_client_cleanup2)
    (
...
bool Client::Login(const std::string& app_key,
    ...
    NIM_SDK_GET_FUNC(nim_client_login)(app_key.c_str(), account.c_str(), password.c_str(), json_extension.c_str(), &CallbackLogin, cb_pointer);
...
bool Client::KickOtherClient(const std::list<std::string>& client_ids) {
    ...
    std::string out = nim::GetJsonStringWithNoStyled(values);
...
static void CallbackSyncMultiportPushConfig(int rescode, const char* content, const char* json_extension, const void* user_data) {
    CallbackProxy::DoSafeCallback<Client::MultiportPushConfigCallback>(user_data, [=](const Client::MultiportPushConfigCallback& cb) {
        nim_cpp_wrapper_util::Json::Value values;
        bool open = false;
        if (rescode == kNIMResSuccess && nim::ParseJsonValue(PCharToString(content), values) && values.isObject()) {
...
void Client::RegSyncMultiportPushConfigCb(const MultiportPushConfigCallback& cb, const std::string& json_extension /* = ""*/) {
    ...
    NIM_SDK_GET_FUNC(nim_client_reg_sync_multiport_push_config_cb)
    (json_extension.c_str(), &CallbackSyncMultiportPushConfig, &g_cb_sync_multiport_push_switch_);
}

static void CallbackMultiportPushConfig(int rescode, const char* content, const char* json_extension, const void* user_data) {
    CallbackProxy::DoSafeCallback<Client::MultiportPushConfigCallback>(
        user_data,
        [=](const Client::MultiportPushConfigCallback& cb) {
            nim_cpp_wrapper_util::Json::Value values;
            bool open = false;
            if (rescode == kNIMResSuccess && nim::ParseJsonValue(PCharToString(content), values) && values.isObject()) {
                open = values[kNIMMultiportPushConfigContentKeyOpen].asInt() == 1;
...
void Client::SetMultiportPushConfigAsync(bool switch_on, const MultiportPushConfigCallback& cb, const std::string& json_extension /* = ""*/) {
    ...
    NIM_SDK_GET_FUNC(nim_client_set_multiport_push_config)
    (json_str.c_str(), json_extension.c_str(), &CallbackMultiportPushConfig, cb_pointer);
}
...
void Client::GetMultiportPushConfigAsync(const MultiportPushConfigCallback& cb, const std::string& json_extension /* = ""*/) {
    ...
    NIM_SDK_GET_FUNC(nim_client_get_multiport_push_config)(json_extension.c_str(), &CallbackMultiportPushConfig, cb_pointer);
}
