...
bool ParseMessage(const std::string& msg_json, IMMessage& message) {
    ...
    if (nim::ParseJsonValue(msg_json, values) && values.isObject()) {
...
void ParseMessage(const nim_cpp_wrapper_util::Json::Value& msg_json, IMMessage& message) {
    ...
    if (msg_json.isMember(kNIMMsgKeyServerExt))
        message.server_ext_ = msg_json[kNIMMsgKeyServerExt].asString();
