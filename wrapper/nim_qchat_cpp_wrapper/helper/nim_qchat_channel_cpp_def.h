...
struct NIM_QCHAT_SDK_CPPWRAPPER_DLL_API QChatChannelInfo {
    ...
    QChatChannelInfo(const NIMQChatChannelInfo& c_param) {
        ...
        visitor_mode = c_param.visitor_mode;
    ...
    NIMQChatChannelInfo ToCParam() const {
        ...
        c_param.visitor_mode = visitor_mode;
...
struct NIM_QCHAT_SDK_CPPWRAPPER_DLL_API QChatChannelCategoryInfo {
    ...
    QChatChannelCategoryInfo(const NIMQChatChannelCategoryInfo& c_param) {
        ...
        create_time = c_param.create_time;
        update_time = c_param.update_time;
        channel_number = c_param.channel_number;
    }
    NIMQChatChannelCategoryInfo ToCParam() const {
        ...
        c_param.create_time = create_time;
        c_param.update_time = update_time;
        c_param.channel_number = channel_number;
        return c_param;
    }
};
...
struct NIM_QCHAT_SDK_CPPWRAPPER_DLL_API QChatChannelGetWhiteBlackRolesPageResp {
    ...
    QChatPageInfo page_info{};
    /// 身份组列表
    std::vector<QChatServerRoleInfo> roles{};
...
struct NIM_QCHAT_SDK_CPPWRAPPER_DLL_API QChatChannelSubscribeAsVisitorResp {
    /// 操作结果, 参考NIMResCode
    uint32_t res_code{0};
    /// 失败列表
    std::vector<NIMQChatChannelIDInfo> failed_channels{};
    ...
    NIMQChatChannelSubscribeAsVisitorResp ToCParam() const {
        NIMQChatChannelSubscribeAsVisitorResp c_resp;
        c_resp.res_code = res_code;
        c_resp.failed_channels = const_cast<NIMQChatChannelIDInfo*>(failed_channels.data());
        c_resp.failed_channels_count = failed_channels.size();
        return c_resp;
...
struct NIM_QCHAT_SDK_CPPWRAPPER_DLL_API QChatChannelUpdateParam {
    ...
    NIMQChatChannelUpdateParam ToCParam() const {
        ...
        param.visitor_mode = visitor_mode;
        param.anti_spam_info = anti_spam_info.ToCParam();
        return param;
    }
};
...
struct NIM_QCHAT_SDK_CPPWRAPPER_DLL_API QChatChannelSubscribeAsVisitorParam {
    /// 回调函数
    ...
    QChatChannelSubscribeAsVisitorParam(const NIMQChatChannelSubscribeAsVisitorParam& c_param) {
        ...
        cb = [c_callback = c_param.cb, user_data = c_param.user_data](const QChatChannelSubscribeAsVisitorResp& response) {
            auto c_response = response.ToCParam();
            c_response.user_data = user_data;
            c_callback(&c_response);
        };
    ...
    NIMQChatChannelSubscribeAsVisitorParam ToCParam() const {
        auto cb_holder = new ChannelSubscribeAsVisitorCallback(cb);
        NIMQChatChannelSubscribeAsVisitorParam param = {};
        param.user_data = cb_holder;
        param.cb = [](const NIMQChatChannelSubscribeAsVisitorResp* resp) {
            InvokeCallback<ChannelSubscribeAsVisitorCallback, NIMQChatChannelSubscribeAsVisitorResp>(resp);
        };
        param.ope_type = ope_type;
        param.sub_type = sub_type;
        param.id_infos_count = id_infos.size();
