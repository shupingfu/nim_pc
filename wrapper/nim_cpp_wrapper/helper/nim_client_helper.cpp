...
SDKConfig::SDKConfig()
    ...
    , session_ignore_msg_types_({kNIMMessageTypeUnknown})
    ...
    , disable_app_nap_(true)
    , cache_session_data_when_delete_(false)
    , sdk_type(kNIMSDKTypeNative)
    , sdk_human_version("")
    , mock_refer_("")
    , mock_ua_("")
    , fcs_auth_type_(0)
    , custom_enable_fcs_(true)
    , custom_client_type_(0) {}
void SDKConfig::SetCustomClientType(int type) {
    custom_client_type_ = type;
}
int SDKConfig::GetCustomClientType() const {
    return custom_client_type_;
}
