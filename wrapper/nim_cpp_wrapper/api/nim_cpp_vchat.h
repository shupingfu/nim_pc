...
public:
    ...
    typedef std::function<void(const std::string& accid, uint64_t time, const char* data, uint32_t size, int32_t channels, int32_t rate)>
        /// 远端音频数据回调
        RemoteAudioDataCallback;
     ...
     * @return void 无返回值
     */
    static void StartDevice(nim::NIMDeviceType type,
     ...
     * @param[in] json_info Json string 扩展，kNIMVChatUids成员id列表,kNIMVChatCustomVideo自主视频数据和kNIMVChatCustomAudio自主音频
     * 如{"uids":["uid_temp"],"custom_video":1, "custom_audio":1}
     ...
     * @param[in] json_extension Json string 扩展，kNIMVChatCustomVideo自主视频数据和kNIMVChatCustomAudio自主音频 如{"custom_video":1,
     * "custom_audio":1}
    ...
    /** @fn static bool CustomVideoData(uint64_t time,
                                        const char* data,
                                        unsigned int size,
                                        unsigned int width,
                                        unsigned int height,
     ...
     * @return bool true 调用成功，false 调用失败
     */
    static bool CustomVideoData(uint64_t time,
        const char* data,
        unsigned int size,
        unsigned int width,
        unsigned int height,
    ...
    /** @fn static bool AccompanyingSound(unsigned char id,
                                          uint64_t time,
                                          const char* data,
                                          unsigned int size,
                                          unsigned int rate,
     ...
     * @return bool true 调用成功，false 调用失败
     */
    static bool AccompanyingSound(unsigned char id,
        uint64_t time,
        const char* data,
        unsigned int size,
        unsigned int rate,
     ...
     * 房间配置信息，详见nim_vchat_def.h，eg.{"room_config":{"rtmpTasks":[{"taskId":"stream_1","streamUrl":"rtmp://test.url","layoutMode":"0","layoutPara":{"k1":"v1"},"record":true,"accid":"123"}]}}
     * @param[in] cb 结果回调见nim_vchat_def.h，返回的json_extension无效
     * @return void 无返回值
     * @note 错误码    200:成功
     *                417:提示已经创建好频道
