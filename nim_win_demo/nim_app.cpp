...
void MainThread::Init() {
    ...
    if (language_setting.m_strResource.compare(L"lang\\zh_CN") == 0)
        ...
        language_setting.m_enumType = ui::LanguageType::Simplified_Chinese;
    ui::GlobalManager::Startup(theme_dir + L"resources\\", ExternCtrlManager::CreateExternCtrl, adapt_api, L"themes\\default", language_setting);
...
bool NimAPP::InitNim(const std::string& server_conf_file_path) {
    ...
    //初始化聊天室
    {
        ...
        if (ret) {
            ...
            nim_chatroom::ChatRoom::SetCallbackFunction([](const StdClosure& task) { nbase::ThreadManager::PostTask(ThreadId::kThreadUI, task); });
#endif
        }
        assert(ret);
    }
...
//#ifdef _DEBUG
//	//cef_path += L"cef_debug"; // 现在即使在debug模式下也使用cef
// release版本的dll，为了屏蔽掉cef退出时的中断，如果不需要调试cef的功能不需要使用debug版本的dll 	redist_packages_path += L"redist_packages";
// #else 	redist_packages_path += L"redist_packages"; #endif 	if (!nbase::FilePathIsExist(redist_packages_path, true))
//	{
...
int NimAPP::InitEnvironment(HINSTANCE hInst, HINSTANCE hPrevInst, LPWSTR lpszCmdLine, int nCmdShow) {
    ...
    if (!nim_cef::CefManager::GetInstance()->Initialize(QPath::GetNimAppDataDir(APPDATA_DIR), settings,
                                                        atoi(GetConfigValue("kNIMCefOsrEnabled").c_str()) > 0))
        return 0;
    return 1;
}
void NimAPP::InitGlobalConfig(const std::string& server_conf_file_path, std::string& app_key, nim::SDKConfig& config) {
    ...
    if (app_sdk::AppSDKInterface::HasconfigValue(nim::kNIMAnimatedImageThumbnailEnabled))
        config.animated_image_thumbnail_enabled_ =
            (app_sdk::AppSDKInterface::GetConfigValue(nim::kNIMAnimatedImageThumbnailEnabled).compare("0") != 0);
    ...
    if (app_sdk::AppSDKInterface::HasconfigValue(nim::kNIMEnableUserDataFileDefRestoreProc))
        config.enable_user_datafile_defrestoreproc_ =
            (app_sdk::AppSDKInterface::GetConfigValue(nim::kNIMEnableUserDataFileDefRestoreProc).compare("0") != 0);
    ...
    //融合存储
    if (app_sdk::AppSDKInterface::HasconfigValue(nim::kNIMFcsAuthType))
        config.fcs_auth_type_ = std::atoi(app_sdk::AppSDKInterface::GetConfigValue(nim::kNIMFcsAuthType).c_str());
    if (app_sdk::AppSDKInterface::HasconfigValue(nim::kNIMFcsCustomEnable))
        config.custom_enable_fcs_ = (app_sdk::AppSDKInterface::GetConfigValue(nim::kNIMFcsCustomEnable).compare("0") != 0);
    if (app_sdk::AppSDKInterface::HasconfigValue(nim::kNIMMockUA))
        config.mock_ua_ = app_sdk::AppSDKInterface::GetConfigValue(nim::kNIMMockUA);
    if (app_sdk::AppSDKInterface::HasconfigValue(nim::kNIMMockRefer))
        config.mock_refer_ = app_sdk::AppSDKInterface::GetConfigValue(nim::kNIMMockRefer);


    // config.lbs_address_ = "https://lbs.netease.im/lbs/conf.jsp";
    // config.default_link_address_ = "link-sg.netease.im:7000";
    // config.use_https_ = true;
    // config.lbs_backup_address_ = {"https://lbs.netease.im/lbs/conf.jsp"};
    // config.nos_lbs_address_ = "https://wanproxy.127.net/lbs?version=1.0";
    //config.default_nos_upload_address_ = "http://nosup-hz1.127.net";
    //config.default_nos_upload_host_ = "nosup-hz1.127.net";
    //config.nos_download_address_ = "{bucket}.nosdn.127.net/{object}";
}
