...
namespace nim_comp
{
	...
	nim::UserNameCard	ProfileBox::GetNameCard() const
	{
		return m_uinfo;
	}
	...
	void ProfileBox::OnUserInfoChange(const std::list<nim::UserNameCard>& uinfos)
	{
	...
	void nim_comp::ProfileBox::OnMiscUInfoChange(const std::list<nim::UserNameCard>& uinfos)
	{
		for (auto info : uinfos)
		{
			if (m_uinfo.GetAccId() == info.GetAccId())
