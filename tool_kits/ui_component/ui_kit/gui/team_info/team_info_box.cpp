...
void TeamInfoBox::OnUserInfoChange(const std::list<nim::UserNameCard>& uinfos)
{
	for (auto iter = uinfos.cbegin(); iter != uinfos.cend(); iter++)
	{
		...
		if (item)
		{
			...
			//if (!iter->ExistValue(nim::kUserNameCardKeyName)) {
			//	std::string nick = team_member_list_.at(iter->GetAccId())->GetNick();
			//	if (!nick.empty()) {
			//		show_name_label->SetText(nbase::UTF8ToUTF16(nick));
			//	}
			//	else {
			//		show_name_label->SetText(UserService::GetInstance()->GetUserName(iter->GetAccId()));
			//	}
			...
			//else
			//{
			//	show_name_label->SetText(nbase::UTF8ToUTF16(nim::kUserNameCardKeyName));
			//}
			...
			if (iter->ExistValue(nim::kUserNameCardKeyName) && team_member_list_.at(iter->GetAccId())->GetNick().empty())
				show_name_label->SetText(UserService::GetInstance()->GetUserName(iter->GetAccId()));
				 //show_name_label = (ui::Label*)item->FindSubControl(L"show_name");
		}
	}
}
