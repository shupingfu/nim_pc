...
bool SessionBox::Notify(ui::EventArgs* param)
{
	...
	else if (param->Type == ui::kEventNotify)
	{
		...
		else if (param->wParam == BET_REMOVE_MEMBER) 
		{
             ...
             //                (MemberManagerForm*)WindowsManager::GetInstance()->GetWindow(MemberManagerForm::kClassName, nbase::UTF8ToUTF16(rm.uid_));
		...
		else if (param->wParam == BET_MUTE_MEMBER) 
		{
			...
			nim::Team::MuteMemberAsync(member_info_->GetTeamID(), member_info_->GetAccountID(), !member_info_->IsMute(),nbase::Bind(&TeamCallback::OnTeamEventCallback, std::placeholders::_1));
		}
	}
