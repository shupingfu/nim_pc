...
namespace nim_comp
{
    ...
    void SessionBox::InitSessionBox()
    {
        ...
        ip_info_ = (Label*)FindSubControl(L"ip_info");
        ...
        //btn_clear_input_ = static_cast<ui::Button*>(FindSubControl(L"clear_input"));
    ...
    MsgBubbleItem* SessionBox::ShowMsg(const nim::IMMessage &msg, bool show_time)
    {
        ...
        //else
            ...
            if (iter->second->GetUserType() == nim::kNIMTeamUserTypeCreator)
            {
                back_name = "  群主";
    ...
    void SessionBox::SendVideo(const std::string& file_path, const std::string& file_ext)
    {
        ...
        AddSendingMsg(msg);
        std::string json_msg = nim::Talk::CreateVideoMessage(msg.receiver_accid_, msg.session_type_, msg.client_msg_id_, video, msg.local_res_path_, nim::MessageSetting(), msg.timetag_);
        nim::Talk::SendMsg(json_msg);
    }
    ...
    void SessionBox::SetIpInfo(std::string accid) {
        ...
        if (ex_json.isObject() && ex_json.isMember("loginIp") && ex_json.isMember("ipLocation")) {
            ...
            ip_info_->SetText(nbase::UTF8ToUTF16("[" + value + "]" + "[" + location + "]"));
        }
    }
