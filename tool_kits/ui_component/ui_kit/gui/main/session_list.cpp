...
SessionList::SessionList(ui::VirtualListBox* session_list, ui::ListBox* top_session_list) {
    ...
    auto changed_cb = session_list_->ToWeakCallback(
        std::bind(&SessionList::OnSessionChangeCallback, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
    ...
    auto userphoto_ready_cb = session_list_->ToWeakCallback(
        std::bind(&SessionList::OnUserPhotoReady, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
    ...
    auto receive_event_cb = session_list_->ToWeakCallback(
        std::bind(&SessionList::OnReceiveEvent, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
    unregister_cb.Add(SubscribeEventManager::GetInstance()->RegReceiveEventCallback(receive_event_cb));

    auto notify_cb =
        session_list_->ToWeakCallback(std::bind(&SessionList::OnNotifyChangeCallback, this, std::placeholders::_1, std::placeholders::_2));
    unregister_cb.Add(MuteBlackService::GetInstance()->RegSyncSetMuteCallback(notify_cb));

    auto team_notify_cb = session_list_->ToWeakCallback(
        std::bind(&SessionList::OnTeamNotificationModeChangeCallback, this, std::placeholders::_1, std::placeholders::_2));
    unregister_cb.Add(TeamService::GetInstance()->RegChangeTeamNotification(team_notify_cb));

    auto friend_list_change_cb =
        session_list_->ToWeakCallback(nbase::Bind(&SessionList::OnFriendListChange, this, std::placeholders::_1, std::placeholders::_2));
...
bool SessionList::OnReturnEventsClick(ui::EventArgs* param) {
    if (param->Type == kEventReturn) {
        ...
        assert(item);
        if (item) {
            SessionManager::GetInstance()->OpenSessionBox(item->GetUTF8DataID(),
                                                          item->GetIsTeam() ? nim::kNIMSessionTypeTeam : nim::kNIMSessionTypeP2P);
        }
    }
...
int SessionList::AddSessionItemData(const nim::SessionData& item_data) {
    ...
    if (session_list_data_[item_data_new->id_] == nullptr)
        ...
        *session_list_data_[item_data_new->id_] = *item_data_new;
    auto sort_it = std::find_if(session_list_sort_data_.begin(), session_list_sort_data_.end(),
                                [&](const std::shared_ptr<nim::SessionData>& item) { return item->id_.compare(item_data_new->id_) == 0; });
...
bool SessionList::DeleteSessionItemData(const std::string& session_id) {
    ...
    auto sort_it = std::find_if(session_list_sort_data_.begin(), session_list_sort_data_.end(),
                                [&](const std::shared_ptr<nim::SessionData>& item) { return item->id_.compare(session_id) == 0; });
    if (sort_it != session_list_sort_data_.end())
...
void SessionList::OnNotifyChangeCallback(std::string id, bool mute) {
    ...
    if (it != session_list_data_.end() &&
        (it->second->type_ == nim::NIMSessionType::kNIMSessionTypeTeam || it->second->type_ == nim::NIMSessionType::kNIMSessionTypeSuperTeam)) {
        SessionItem* item = dynamic_cast<SessionItem*>(session_list_->FindSubControl(wid));
        if (item && !item->GetIsTeam())
            item->SetMute(mute);
    }
...
UnregisterCallback SessionList::RegUnreadCountChange(const OnUnreadCountChangeCallback& callback) {
    ...
    auto cb = ToWeakCallback([this, cb_id]() { unread_count_change_cb_list_.erase(cb_id); });
    return cb;
}
UnregisterCallback SessionList::RegAddItem(const OnAddItemCallback& callback) {
    ...
    auto cb = ToWeakCallback([this, cb_id]() { add_item_cb_list_.erase(cb_id); });
    return cb;
}
...
UnregisterCallback SessionList::RegRemoveItem(const OnRemoveItemCallback& callback) {
    ...
    auto cb = ToWeakCallback([this, cb_id]() { remove_item_cb_list_.erase(cb_id); });
    return cb;
}
...
void SessionList::UpdateMultispotUI() {
    ...
    MultispotForm* form =
        dynamic_cast<MultispotForm*>(WindowsManager::GetInstance()->GetWindow(MultispotForm::kClassName, MultispotForm::kClassName));
    if (form) {
        if (map_multispot_infos_.size() > 0) {
    ...
    switch (data.command_) {
        ...
        case nim::kNIMSessionCommandMsgDeleted: {
            if (data.last_updated_msg_) {
                ...
                if (data.unread_count_ == 0 || SessionManager::GetInstance()->IsSessionBoxActive(session)) {
                    ResetUnreadCount(data.id_);
                }
                if (session && data.command_ == nim::kNIMSessionCommandUpdate && data.msg_status_ == nim::kNIMMsgLogStatusUnread) {
        ...
        case nim::kNIMSessionCommandAllTeamMsgDeleted: {
            ...
            for (int i = session_list_->GetCount() - 1; i >= 0; i--) {
                SessionItem* item = dynamic_cast<SessionItem*>(session_list_->GetItemAt(i));
                if (item) {
                    if (data.command_ == nim::kNIMSessionCommandAllMsgDeleted ||
                        (item->GetIsTeam() == (data.command_ == nim::kNIMSessionCommandAllTeamMsgDeleted))) {
                        item->ClearMsg();
                    }
                }
            }
...
bool SessionList::OnItemNotify(ui::EventArgs* msg) {
    if (msg->Type == kEventNotify) {
        ...
        if (msg->wParam == SET_DELETE) {
            ...
            Post2UI(ToWeakCallback([this, id]() { DeleteSessionItem(id); }));
        } else if (msg->wParam == SET_TO_TOP || msg->wParam == SET_CANCEL_FROM_TOP) {
            Post2UI(ToWeakCallback([this]() { SortSessionItems(true); }));
        }
    }
    return true;
}
void SessionList::SortSessionItems(bool sel_force_visible) {
    ...
    std::sort(session_list_sort_data_.begin(), session_list_sort_data_.end(),
              [](const std::shared_ptr<nim::SessionData>& item1, const std::shared_ptr<nim::SessionData>& item2) {
    ...
    std::sort(session_list_sort_data_.begin(), session_list_sort_data_.end(),
              [](const std::shared_ptr<nim::SessionData>& item1, const std::shared_ptr<nim::SessionData>& item2) {
                  if (item1->placed_on_top_ && item2->placed_on_top_)
                      return false;
                  return item1->placed_on_top_;
              });
    session_list_->Refresh();
