...
void VideoPlayerBitmapControl::Paint(IRenderContext* pRender, const UiRect& rcPaint) {
    ...
    try {
        ...
        for (auto it = m_items.begin(); it != m_items.end(); it++) {
            ...
            if (!::IntersectRect(&m_rcPaint, &rcPaint, &controlPos))
                continue;
...
void VideoPlayerBitmapControl::Refresh() {
    ...
    ret = libyuv::ARGBScale((const uint8*)image_data_.c_str(), image_width_ * 4, image_width_, image_height_, (uint8*)data_.c_str() + shift,
                            item_w * 4, clip_w, clip_h, libyuv::kFilterBox) >= 0;
    if (ret) {
        width_ = item_w;
        height_ = item_h;
        updated_ = false;
